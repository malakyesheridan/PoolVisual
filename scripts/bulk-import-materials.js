...
function parseProduct(html, url) {
  ...
  try {
    ...
    const imgPatterns = [
      /<img[^>]*class="[^"]*product[^"]*"[^>]*src="(https?:\/\/[^"]+\.(?:jpg|jpeg|png|webp)[^"]*)"[^>]*>/i,
      /<img[^>]*src="(https?:\/\/[^"]+\.(?:jpg|jpeg|png|webp)[^"]*)"[^>]*alt="[^"]*product[^"]*"[^>]*>/i,
      /<img[^>]*src="(https?:\/\/[^"]+product[^"]*\.(?:jpg|jpeg|png|webp)[^"]*)"[^>]*>/i
    ];
    
    for (const pattern of imgPatterns) {
      const match = html.match(pattern);
      if (match) {
        data.textureUrl = match[1];
        data.thumbnailUrl = match[1];
        break;
      }
    }
...
async function createMaterial(data) {
  try {
    const payload = {
      ...
      unit: data.unit,
      price: data.price,
      cost: data.price ? (data.price * 0.85).toFixed(2) : null,
      tileWidthMm: data.tileSize,
      tileHeightMm: data.tileSize,
      sheetWidthMm: data.sheetSize?.width,
      sheetHeightMm: data.sheetSize?.height,
