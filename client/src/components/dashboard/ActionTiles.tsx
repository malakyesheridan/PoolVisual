...
export function ActionTiles({ jobs, quotes, isLoading = false, className = '' }: ActionTilesProps) {
  ...
  // Group quotes by jobId for quick lookup (must be before early returns)
  ...
  // Attach quotes to jobs (must be before early returns)
  ...
  // Calculate action counts (must be before early returns)
  ...
  // Only render for trades (after all hooks)
  if (!isTrades) {
    return null;
  }
