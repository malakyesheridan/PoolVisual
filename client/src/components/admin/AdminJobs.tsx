...
const JOB_STATUSES = ['new', 'estimating', 'sent', 'accepted', 'declined', 'scheduled', 'completed'];
const INDUSTRIES = ['pool', 'landscaping', 'building', 'electrical', 'plumbing', 'real_estate', 'other'];
...
export function AdminJobs() {
  ...
  const { data, isLoading, isError, error } = useQuery({
    queryKey: ['admin', 'jobs', page, search, statusFilter, industryFilter],
        ...
        <CardContent>
          ...
          <div className="mb-6 space-y-4">
            <div className="relative">
              ...
              <Input
                placeholder="Search jobs by client name, email, or address..."
                value={search}
                onChange={(e) => {
                  ...
                  jobs.map((job: any) => (
                    <tr key={job.id} className="hover:bg-slate-50">
                      <td className="px-4 py-3">
                        <div>
                          <div className="font-medium text-slate-900">{job.clientName || 'N/A'}</div>
                          {job.clientEmail && (
                            <div className="text-sm text-slate-500">{job.clientEmail}</div>
                          )}
                          {job.address && (
