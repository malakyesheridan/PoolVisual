export async function createMaterialV2(input: any) {
  ...
  console.log('[v2/materials] → POST /api/v2/materials', payload);
  ...
  console.log('[v2/materials] ←', res.status, data);
  if (!res.ok) {
...
// Test probe endpoint
export async function testProbeV2() {
  const res = await fetch('/api/v2/_probe', {
    credentials: 'include'
  });
  return res.json();
}